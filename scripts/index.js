...
const contentDetails = async (vedioId) => {
  try {
    ...
    localStorage.setItem("videoDataForNextPage", JSON.stringify(videoData));
    return data.items[0];
  } catch (err) {
    console.log("err: ", err);
  }
...
const appendData = (data) => {
  ...
  data.forEach(async ({ snippet, id: videoId }, i) => {
    ...
    document.querySelector("#content-container").innerHTML += showTemplate;
  });
};

//!------------------ Sending Video detail tto LS for the video .html pge-0-----------
